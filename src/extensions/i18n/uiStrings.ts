// Centralized UI strings (typed). Only text binding—no behavior changes.
export type UIText = {
  toolbar: {
    addNode: string; createEdge: string; loadAllEdges: string;
    saveModel: string; saveVersion: string; versions: string; help: string;
    importEks: string; exportEks: string; relayout: string;
    showSelf: string; hideSelf: string;
    tt: { addNode: string; createEdge: string; relayout: string };
  };
  filters: {
    title: string; deltaLabel: string; time25: string; time100: string;
    probMin: string; probMax: string; rangeAny: string; rangeBoth: string; clear: string;
    emptyNoMatch: string;
  };
  edge: {
    tooltipTitle: string; tooltipFromTo: string; tooltipDelta: string;
    tooltipFlags: string; tooltipProbs: string;
  };
  empty: { noEdges: string; ctaCreateEdge: string };
  toasts: { saveSuccess: string; saveFail: string; presetApplied: string };
};

export const t: UIText = {
  toolbar: {
    addNode: 'Add Node',
